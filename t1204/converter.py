...
buf += b"\x78\x65\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
# Pad the above line so it's 16 bytes

# Variable is equivalent to one of these two provided below:
...
for x in range(0, len(buf), 16):
#	print("{}".format(buf[x:x+16].encode('hex')))

	block = buf[x:x+16]
	uuid = UUID(bytes_le=block)
	
# Convert to exact print
	print("ImageData({}) = \"{}\"".format(count,str(uuid)))
	count += 1
